...
const Footer = ({ mode }: { mode: Mode }) => {
  ...
  const logoColor =
    theme.palette.mode === 'dark' ? 'var(--mui-palette-common-white)' : 'var(--mui-palette-text-primary)'

  return (
    <footer className={classnames('relative bg-backgroundDefault', frontLayoutClasses.footer)}>
      <div>
          ...
          <Grid container rowSpacing={10} columnSpacing={12}>
            <Grid item xs={12} lg={8}>
              <div className='flex flex-col sm:flex-row items-center gap-6'>
                ...
                <Typography
                  ...
                  sx={{ color: logoColor, opacity: theme.palette.mode === 'dark' ? 0.78 : 0.87 }}
                >
            ...
            <Grid item xs={12} lg={4}>
              <div className='flex flex-row gap-4'>
                {['Home', 'Features', 'Team', 'FAQ', 'Contact Us'].map((item, index) => (
                  <Typography
                    key={index}
                    component={Link}
        ...
        >
          ...
          {!isBreakpointReached && (
            <div className='flex items-center gap-4'>
              <Link href='/pages/misc/terms-of-service' target='_blank' className='text-primary'>
                Terms of Service
              </Link>
              <Link href='/pages/misc/privacy-policy' target='_blank' className='text-primary'>
                Privacy Policy
              </Link>
            </div>
          )}
